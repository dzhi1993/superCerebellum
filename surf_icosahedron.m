...
% 12 - 1442 hexagons
% Revision:
%      v0.1 - add 'medialWall' option to determine 
%             whether the returned icosahedron includes
%             the medial wall or not. - dzhi
% ---------------------------------------------------
freq    = 6;
radius  = 100;
...
medialWall = 1; % 0 - no medial wall; 1 - add medial wall (default)
vararginoptions(varargin,{'freq','radius','res','thres','medialWall'});
cd(atlasDir);
...
nLabel = length(unique(centr)); % without medial wall
...
columnNames     = cell(nLabel,1);
labelNames      = cell(nLabel,1);
